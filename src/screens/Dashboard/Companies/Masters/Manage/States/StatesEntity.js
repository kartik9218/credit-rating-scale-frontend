...
function StatesEntity() {
  ...
  const [title, setTitle] = useState(uuid ? "Edit State" : "Add State");
  ...
  const onFormSubmit = async (ev) => {
    ...
    let url = uuid ? "/v1/states/edit" : "/v1/states/create";
    if (!uuid) {
      delete params.uuid;
    }
    ...
    HTTP_CLIENT(APIFY(url), { params: params })
      .then((response) => {
        if (response["success"]) {
          ...
          navigate(GET_ROUTE_NAME("LIST_STATE"), {
  ...
  const fetchData = async () => {
    ...
    HTTP_CLIENT(APIFY("/v1/states/view"), {
      params: {
        uuid: uuid,
      },
    }).then((response) => {
      if (response["success"]) {
        updateParams("name", response.state.name);
        fetchStates({
        ...
        <CardWrapper
          ...
          headerActionButton={() => {
            return (
              <HasPermissionButton
                color="primary"
                permissions={["/dashboard/company/master/states"]}
                route={GET_ROUTE_NAME("LIST_STATE")}
                text={`Back to States`}
            ...
            <Grid container spacing={1} paddingLeft="1rem" paddingRight="1rem">
              ...
              <Grid item xs={12} sm={6} md={6}>
                ...
                <ArgonSelect
                  ...
                  onChange={(e) => updateParams("country_uuid", e.value)}
                  required
                />
              </Grid>
            </Grid>
