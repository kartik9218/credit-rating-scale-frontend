...
function Departments() {
  ...
  const fetchDepartments = async () => {
    HTTP_CLIENT(APIFY("/v1/departments"), { params: {} }).then((response) => {
      ...
      setColumns([
        ...
        { accessor: "name", Header: "Name" },
        {
          ...
          Header: "HOD",
          Cell: (row) => {
            return <>{row.cell.value && row.cell.value.full_name}</>;
          },
        },
        {
          accessor: "is_active",
          ...
          Cell: (row) => {
            return (
              <ArgonBox display="Flex" flexDirection="row" justifyContent="space-between">
                <HasPermissionButton
                  color="primary"
                  permissions={["/dashboard/company/master/departments/edit"]}
                  route={GET_ROUTE_NAME("EDIT_DEPARTMENT", { uuid: row.cell.value })}
                  text={`Edit`}
                  icon={<Edit />}
                />
              </ArgonBox>
      ...
      departments.forEach((department, key) => {
        departments[key].id = key + 1;
      })
      setRows(departments);
    });
  ...
  useEffect(() => {
    SET_PAGE_TITLE(`Manage Departments`);
  ...
  return (
    <DashboardLayout breadcrumbTitle="Manage Departments">
      <CardWrapper
        ...
        headerActionButton={() => {
          return (
            <HasPermissionButton
              ...
              route={GET_ROUTE_NAME("ADD_DEPARTMENT")}
