...
function Sectors() {
  ...
  const fetchSectors = async () => {
    ...
    HTTP_CLIENT(APIFY("/v1/sectors"), { params: {} }).then((response) => {
      ...
      setColumns([
        ...
        {
          accessor: "macro_economic_indicator",
          Header: "Macro Economic Indicator",
          Cell: (row) => {
          ...
          Cell: (row) => {
            return (
              <ArgonBox display="Flex" flexDirection="row" justifyContent="space-between">
                <HasPermissionButton
                  color="primary"
                  permissions={["/dashboard/company/master/sectors/edit"]}
                  route={GET_ROUTE_NAME("EDIT_SECTOR", { uuid: row.cell.value })}
                  text={`Edit`}
                  icon={<Edit />}
                />
              </ArgonBox>
      ...
      sectors.forEach((types, key) => {
        sectors[key].id = key + 1;
      })
      setRows(sectors);
    });
  ...
  return (
    <DashboardLayout breadcrumbTitle="Manage Sectors">
      <CardWrapper
        headerTitle="Manage Sectors"
        headerActionButton={() => {
          return (
            <HasPermissionButton
              color="primary"
              permissions={["/dashboard/company/master/sectors/create"]}
              route={GET_ROUTE_NAME("ADD_SECTOR")}
              text={`Add new Sector`}
              icon={<Add />}
            />
          );
