...
function InteractionTypeEntity() {
  const navigate = useNavigate();
  const uuid = GET_QUERY("uuid");
  const [title, setTitle] = useState(uuid ? "Edit Interaction Type" : "Add Interaction Type");
  ...
  const fetchData = async () => {
    ...
    HTTP_CLIENT(APIFY("/v1/interaction_type"), {
      params: {
        uuid: uuid,
      },
    }).then((response) => {
      if (response["success"] && response.interaction_type.length > 0) {
        let interaction_type = response.interaction_type[0];
        updateParams("name", interaction_type.name);
        updateParams("is_active", interaction_type.is_active);
        return;
      }
    });
  ...
  const onFormSubmit = async (ev) => {
    ...
    let url = uuid ? "/v1/interaction_type/edit" : "/v1/interaction_type/create";
    if (!uuid) {
      delete params.uuid;
    ...
    HTTP_CLIENT(APIFY(url), { params: params })
      .then((response) => {
        if (response["success"]) {
          ...
          navigate(GET_ROUTE_NAME("LIST_INTERACTION_TYPE"), {
        ...
        <CardWrapper
          ...
          headerActionButton={() => {
            return (
              <HasPermissionButton
                color="primary"
                permissions={["/dashboard/company/master/interaction-types"]}
                route={GET_ROUTE_NAME("LIST_INTERACTION_TYPE")}
                text={`Back to Interaction Types`}
...
export default InteractionTypeEntity;
