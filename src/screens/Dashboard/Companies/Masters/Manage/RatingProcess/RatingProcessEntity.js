...
function RatingProcessEntity() {
  const navigate = useNavigate();
  const uuid = GET_QUERY("uuid");
  const [title, setTitle] = useState(uuid ? "Edit Rating Process" : "Add Rating Process");
  ...
  const fetchData = async () => {
    ...
    HTTP_CLIENT(APIFY("/v1/rating_process/view"), { params: {
      uuid:uuid
    } })
      .then((response) => {
        if (response["success"]) {
          updateParams('name', response.rating_process.name);
          updateParams('is_active', response.rating_process.is_active);
  ...
  const onFormSubmit = async (ev) => {
    ...
    let url = uuid ? "/v1/rating_process/edit" : "/v1/rating_process/create";
    if (!uuid) {
      delete params.uuid;
    ...
    HTTP_CLIENT(APIFY(url), { params: params })
      .then((response) => {
        if (response["success"]) {
          ...
          navigate(GET_ROUTE_NAME("LIST_RATING_PROCESS"), { state: { success: true, type: uuid ? "UPDATE" : "CREATE" } });
          return;
        }
      })
      .catch((err) => {
        ...
        <CardWrapper
          ...
          headerActionButton={() => {
            return (
              <HasPermissionButton
                color="primary"
                permissions={["/dashboard/company/master/rating-process"]}
                route={GET_ROUTE_NAME("LIST_RATING_PROCESS")}
...
export default RatingProcessEntity;
