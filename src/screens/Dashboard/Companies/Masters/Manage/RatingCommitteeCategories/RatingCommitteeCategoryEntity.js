...
function RatingCommitteeCategoryEntity() {
  const navigate = useNavigate();
  const uuid = GET_QUERY("uuid");
  const [title, setTitle] = useState(
    uuid ? "Edit Rating Committee Category" : "Add Rating Committee Category"
  );
  ...
  const fetchData = async () => {
    ...
    HTTP_CLIENT(APIFY("/v1/rating_committee_meeting_categories/view"), {
      params: {
        uuid: uuid,
      },
    }).then((response) => {
      if (response["success"]) {
        updateParams("name", response.rating_committee_meeting_category.name);
        updateParams("description", response.rating_committee_meeting_category.description);
        updateParams("is_active", response.rating_committee_meeting_category.is_active);
  ...
  const onFormSubmit = async (ev) => {
    ...
    let url = uuid
      ? "/v1/rating_committee_meeting_categories/edit"
      : "/v1/rating_committee_meeting_categories/create";
    if (!uuid) {
      delete params.uuid;
    ...
    HTTP_CLIENT(APIFY(url), { params: params })
      .then((response) => {
        if (response["success"]) {
          ...
          navigate(GET_ROUTE_NAME("LIST_RATING_COMMITTEE_CATEGORIES"), {
        ...
        <CardWrapper
          ...
          headerActionButton={() => {
            return (
              <HasPermissionButton
                color="primary"
                permissions={["/dashboard/company/master/rating-committee-categories"]}
                route={GET_ROUTE_NAME("LIST_RATING_COMMITTEE_CATEGORIES")}
                text={`Back to Rating Committee Categories`}
            ...
            <Grid container spacing={1} paddingLeft="1rem" paddingRight="1rem">
              <Grid item xs={12} sm={6} md={6}>
                <ArgonBox mb={1} ml={0.5} lineHeight={0} display="inline-block">
                  ...
                  >
                    Rating Committee Category Name*
                  </ArgonTypography>
                </ArgonBox>
                <ArgonInput
                  type="text"
                  name="rating_committee_category_name"
                  label="Name*"
                  placeholder="Enter Rating Committee Category Name"
...
export default RatingCommitteeCategoryEntity;
